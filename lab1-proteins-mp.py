...
# MULTIPROCESSING VERSION (using multiprocessing.Pool)
...
#Update clusters after random initialization
...
def compute_wcss_for_k(args):
    X, k = args
    centroids, clusters = kmeans(X, k)
    return wcss_computation(X, centroids, clusters)
...
if __name__ == "__main__":
    ...
    # Use multiprocessing for computing WCSS across different k values
    with Pool(processes=cpu_count()) as pool:
        wcss = pool.map(compute_wcss_for_k, [(X, k) for k in K_range])

    # Find optimal k (basic elbow method heuristic: largest drop)
    ...
    # Heatmap using the values of clusters' centroids
    plt.figure()
